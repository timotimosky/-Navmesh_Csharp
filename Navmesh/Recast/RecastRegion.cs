...
//	import static org.recast4j.recast.RecastConstants.RC_NULL_AREA;
	...
	public class RecastRegion
	{
		...
		internal const int RC_NULL_NEI = 0xffff;

		internal class SweepSpan
		{
			internal int rid; // row id
			internal int id; // region id
			internal int ns; // number samples
			internal int nei; // neighbour id
		}
		...
		private static int[] expandRegions(int maxIter, int level, CompactHeightfield chf, int[] srcReg, int[] srcDist, int[] dstReg, int[] dstDist, List<int> stack, bool fillStack)
		{
			...
			{
				int failed = 0;
				Array.Copy(srcReg, 0, dstReg, 0, chf.spanCount);
				Array.Copy(srcDist, 0, dstDist, 0, chf.spanCount);
		...
		{
			internal int spanCount; // Number of spans belonging to this region
			internal int id; // ID of the region
			internal int areaType; // Are type.
			internal bool remap;
			internal bool visited;
			internal bool overlap;
			internal bool connectsToBorder;
			internal int ymin, ymax;
			internal List<int> connections;
			internal List<int> floors;

			internal Region(int i)
			{
				this.id = i;
				this.ymin = 0xFFFF;
		...
		private static int mergeAndFilterRegions(Context ctx, int minRegionArea, int mergeRegionSize, int maxRegionId, CompactHeightfield chf, int[] srcReg, List<int> overlaps)
		{
			int w = chf.width;
			int h = chf.height;
		...
		private static int mergeAndFilterLayerRegions(Context ctx, int minRegionArea, int maxRegionId, CompactHeightfield chf, int[] srcReg, List<int> overlaps)
		{
			int w = chf.width;
			int h = chf.height;
		...
		public static void buildRegions(Context ctx, CompactHeightfield chf, int borderSize, int minRegionArea, int mergeRegionArea)
		{
			...
			if (overlaps.Count > 0)
			{
				throw new Exception("rcBuildRegions: " + overlaps.Count + " overlapping regions.");
			}
