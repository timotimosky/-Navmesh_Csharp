	...
	public class RecastContour
	{
		...
		private static void simplifyContour(List<int> points, List<int> simplified, float maxError, int maxEdgeLen, int buildFlags)
		{
			...
			if (hasConnections)
			{
				...
				{
					int ii = (i + 1) % ni;
                    bool differentRegs = (points[i * 4 + 3] & RecastConstants.RC_CONTOUR_REG_MASK) != (points[ii * 4 + 3] & RecastConstants.RC_CONTOUR_REG_MASK);
                    bool areaBorders = (points[i * 4 + 3] & RecastConstants.RC_AREA_BORDER) != (points[ii * 4 + 3] & RecastConstants.RC_AREA_BORDER);
			...
			for (int i = 0; i < simplified.Count / 4; ++i)
			{
                ...
                simplified[i * 4 + 3] = (points[ai * 4 + 3] & (RecastConstants.RC_CONTOUR_REG_MASK | RecastConstants.RC_AREA_BORDER)) | (points[bi * 4 + 3] & RecastConstants.RC_BORDER_VERTEX);
			}
		...
		private class CompareHoles : IComparer<ContourHole>
		{
			...
			public virtual int Compare(ContourHole a, ContourHole b)
			{
				if (a.minx == b.minx)
				{
					if (a.minz < b.minz)
		...
		private class CompareDiagDist : IComparer<PotentialDiagonal>
		{
			...
			public virtual int Compare(PotentialDiagonal va, PotentialDiagonal vb)
			{
