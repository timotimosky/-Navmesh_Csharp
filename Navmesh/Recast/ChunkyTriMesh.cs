...
namespace org.recast4j.recast
{
	...
	public class ChunkyTriMesh
	{
		...
		{
			internal readonly float[] bmin = new float[2];
			internal readonly float[] bmax = new float[2];
			internal int i;
		}
		...
		private class CompareItemX : IComparer<BoundsItem>
		{
			...
			public virtual int Compare(BoundsItem a, BoundsItem b)
			{
		...
		internal List<ChunkyTriMeshNode> nodes;
		internal int ntris;
		internal int maxTrisPerChunk;
