...
//	import static org.recast4j.recast.RecastConstants.RC_MESH_NULL_IDX;
	...
	public class RecastMesh
	{
		...
		internal static int VERTEX_BUCKET_COUNT = (1 << 12);

		private class Edge
		{
			internal int[] vert = new int[2];
			internal int[] polyEdge = new int[2];
			internal int[] poly = new int[2];
		...
		private static void buildMeshAdjacency(int[] polys, int npolys, int nverts, int vertsPerPoly)
		{
			...
			for (int i = 0; i < npolys; ++i)
			{
				...
				for (int j = 0; j < vertsPerPoly; ++j)
				{
					...
					int v0 = polys[t + j];
                    int v1 = (j + 1 >= vertsPerPoly || polys[t + j + 1] == RecastConstants.RC_MESH_NULL_IDX) ? polys[t + 0] : polys[t + j + 1];
					if (v0 < v1)
					{
						Edge edge = new Edge();
		...
		private static int computeVertexHash(int x, int y, int z)
		{
			...
			int h3 = unchecked((int)0xcb1ab31f);
		...
		internal static int prev(int i, int n)
		{
			return i - 1 >= 0 ? i - 1 : n - 1;
		}

		internal static int next(int i, int n)
		{
			return i + 1 < n ? i + 1 : 0;
		}
		...
		// line through a to b.
		internal static bool left(int[] verts, int a, int b, int c)
		{
			return area2(verts, a, b, c) < 0;
		}

		internal static bool leftOn(int[] verts, int a, int b, int c)
		{
			return area2(verts, a, b, c) <= 0;
		}
		...
		internal static bool intersect(int[] verts, int a, int b, int c, int d)
		{
		...
		internal static bool vequal(int[] verts, int a, int b)
		{
		...
		private static int triangulate(int n, int[] verts, int[] indices, int[] tris)
		{
			...
			while (n > 3)
			{
				...
				tris[ntris * 3] = indices[i00] & 0x0fffffff;
				tris[ntris * 3 + 1] = indices[i11] & 0x0fffffff;
				tris[ntris * 3 + 2] = indices[i22] & 0x0fffffff;
				ntris++;
				...
				if (diagonal(prev(i00, n), i11, n, verts, indices))
				{
					indices[i00] |= unchecked((int)0x80000000);
				}
				else
				{
					indices[i00] &= 0x0fffffff;
				}

				if (diagonal(i00, next(i11, n), n, verts, indices))
				{
					indices[i11] |= unchecked((int)0x80000000);
				}
				else
				{
					indices[i11] &= 0x0fffffff;
				}
			}
		...
		private static bool canRemoveVertex(Context ctx, PolyMesh mesh, int rem)
		{
			int nvp = mesh.nvp;
		...
		public static PolyMesh buildPolyMesh(Context ctx, ContourSet cset, int nvp)
		{
			...
			for (int i = 0; i < cset.conts.Count; ++i)
			{
				...
				for (int j = 0; j < npolys; ++j)
				{
					...
					if (mesh.npolys > maxTris)
					{
						throw new Exception("rcBuildPolyMesh: Too many polygons " + mesh.npolys + " (max:" + maxTris + ").");
					}
				}
			}
			...
			{
				throw new Exception("rcBuildPolyMesh: The resulting mesh has too many vertices " + mesh.nverts + " (max " + 0xffff + "). Data can be corrupted.");
			}
			if (mesh.npolys > 0xffff)
			{
				throw new Exception("rcBuildPolyMesh: The resulting mesh has too many polygons " + mesh.npolys + " (max " + 0xffff + "). Data can be corrupted.");
			}
		...
		public static PolyMesh mergePolyMeshes(Context ctx, PolyMesh[] meshes, int nmeshes)
		{
			...
			for (int i = 0; i < nmeshes; ++i)
			{
				...
				{
					int v = j * 3;
					int[] inv = addVertex(pmesh.verts[v + 0] + ox, pmesh.verts[v + 1], pmesh.verts[v + 2] + oz, mesh.verts, firstVert, nextVert, mesh.nverts);
